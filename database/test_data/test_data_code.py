...
while restart_count < 5 and has_errors:
    ...
    try:
        if not -8197955095366998348 in succeeded_hashes:  # avoid duplicate inserts
            instance = customer1 = Customer(id=1, name='Alice', credit_limit=5000, balance=299.95)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-8197955095366998348)
    except Exception as e:
        has_errors = True
    ...
    try:
        if not -2449544532251356148 in succeeded_hashes:  # avoid duplicate inserts
            instance = product1 = Product(id=1, name='Gadget', unit_price=99.99)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-2449544532251356148)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -451893336173196052 in succeeded_hashes:  # avoid duplicate inserts
            instance = order1 = Order(id=1, customer_id=1, amount_total=299.95, date_shipped=None, notes='')
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-451893336173196052)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -1906253174214555795 in succeeded_hashes:  # avoid duplicate inserts
            instance = item1 = Item(id=1, order_id=1, product_id=1, quantity=1, unit_price=99.99, amount=99.99)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-1906253174214555795)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
